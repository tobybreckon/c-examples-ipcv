// Example : median filter image / video / camera
...
int main( int argc, char** argv )
{
  ...
  // otherwise default to capture from attached H/W camera
	  ...
	  )
    {
	  ...
	  median = cvCreateImage(cvSize(img->width,img->height),
						   img->depth, img->nChannels);
	  median->origin = img->origin;
	  ...
	  while (keepProcessing) {
		  ...
		  // Gmedian filter image
		  cvSmooth(img, median, CV_MEDIAN, windowSize, 0);

		  // display images in window

		  cvShowImage( windowName1, median );

// Example : moments of largest closed contour in an image / video / camera
	  ...
	  )
    {
	  ...
	  while (keepProcessing) {
		  ...
		  if (largest_contour != NULL){
			...
			// then get and display the 1,1, 1,2, and 2,1 xth, yth order
			// normalised central moments
			// N.B. see notes in manual on orders parameters to use (!)

			sprintf(outputString, "cm11: %.10f  cm12: %.10f  "
			  "cm21: %.10f",
			  cvGetNormalizedCentralMoment(&moments, 1,1) * 100,
			  cvGetNormalizedCentralMoment(&moments, 1,2) * 100,
			  cvGetNormalizedCentralMoment(&moments, 2,1) * 100);

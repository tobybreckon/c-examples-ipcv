// Example : L*a*b channels display of image / video / camera
...
int main( int argc, char** argv )
{
  ...
  IplImage*	LABimg = NULL;   // HAV image object
  CvCapture* capture = NULL; // capture object

  char const * windowName = "RGB Original Image"; 	// window name
  char const * windowNameL = "L Channel"; 		// window name
  char const * windowNameA = "A Channel"; // window name
  char const * windowNameB = "B Channel"; 	// window name
  ...
  // otherwise default to capture from attached H/W camera
	  ...
	  )
    {
	  ...
	  cvNamedWindow(windowNameL, 0);
	  cvNamedWindow(windowNameA, 0);
	  ...
	  LABimg = cvCloneImage(img);

	  IplImage* l_channel = cvCreateImage(cvSize(img->width, img->height),
	  															img->depth, 1);
	  IplImage* a_channel = cvCreateImage(cvSize(img->width, img->height),
	  															img->depth, 1);
	  IplImage* b_channel = cvCreateImage(cvSize(img->width, img->height),
	  															img->depth, 1);
	  l_channel->origin = img->origin;
	  a_channel->origin = img->origin;
	  ...
	  while (keepProcessing) {
		  ...
		  cvSplit(LABimg, l_channel, a_channel, b_channel, NULL);

		  // display images in windows

		  cvShowImage( windowName, img );
		  cvShowImage( windowNameL, l_channel );
		  cvShowImage( windowNameA, a_channel );
